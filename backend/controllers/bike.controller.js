...
exports.getAllBikes = catchAsync(async (req, res, next) => {
	...
	if (req.query.coords) {
		const [longitude, latitude] = req.query.coords
			.split(",")
			.map(parseFloat);
...
exports.createBike = catchAsync(async (req, res, next) => {
	...
	if (
		...
		return next(
			new AppError(
				"Please provide all required bike details including model, category, prices, location, and at least one image URL.",
				400
			)
		);
	}
